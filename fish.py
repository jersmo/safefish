...
class Fish:
    """Roblox robot common functions."""

    @classmethod
    def main(cls):
        #  Set the proper helper class based on operating system
        cls.windows = True if os.name == 'nt' else False
        cls.m = None
        if cls.windows:
            cls.m = WindowsHelper()
        else:
        ...
        # Delay before clicking into roblox
        time.sleep(1)

        def check_air_bubbles(counter):
            s = m.screenshot(counter)
            color = (68, 252, 234)
        ...
        def sell_fish():
            ...
            # Click backpack
            m.click(random.randint(1058, 1065), random.randint(915, 925))
            # Wait for green sell box
            m.wait_for_color(977, 353, (84, 234, 52), 2)
...
class WindowsHelper:
    ...
    def __init__(cls):
        ...
        cls.mkey.enable_failsafekill('ctrl+c')

    def click(self, x, y):
        try:
            self.mkey.left_click_xy_natural(
                x, y,
                ...
                percent=99,  # the lower, the straighter the mouse movement
            )
        except:
    ...
    def screenshot(self, counter=0):
        return pyautogui.screenshot()

    def wait_for_color(self, x, y, color, timeout):
        """Wait for color at a coordinate."""
        now = time.time()
        while time.time() - now < timeout:
    ...
    def wait_for_no_color(self, x, y, color, timeout):
        """Wait until there is not the specified color at coordinates"""
        now = time.time()
        while time.time() - now < timeout:
    ...
    def wait_for_color_in_area(self, x1, x2, y1, y2, color, timeout):
        """Waits for color in a boxed area"""
        now = time.time()
        while time.time() - now < timeout:
    ...
    def wait_for_no_color_in_area(self, x1, x2, y1, y2, color, timeout):
        """Waits until color not detected in a specified boxed area"""
        now = time.time()
        while time.time() - now < timeout:
            if not self.wait_for_color_in_area(x1, x2, y1, y2, color, .25):
                return True
        # Return False at end of the loop
        return False
...
class MacHelper:
    ...
    def screenshot(self, counter=0):
        ...
        monitor = self.sct.monitors[1]  # or a region
        # Grab the data
        sct_img = self.sct.grab(monitor)
        # Generate the PNG
        img = self._convert_to_srgb(pil.Image.frombytes("RGB", sct_img.size, sct_img.bgra, "raw", "BGRX"))
